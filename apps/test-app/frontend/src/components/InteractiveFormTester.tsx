...
export const InteractiveFormTester: React.FC<Props> = ({ entity }) => {
  ...
  // Map schema fields by key for constant time lookup
  const schemaMap: Record<string, SchemaField> = {};
  if (entity.schema) {
    Object.assign(schemaMap, entity.schema);
  ...
  return (
    <Paper sx={{ p: 2, background: '#0a192f', border: '1px solid #1e2d4a' }}>
            ...
            <AccordionDetails>
              <Grid container spacing={2}>
                {section.fields.map((fKey) => {
                  const schemaField = schemaMap[fKey];
                  ...
                  return (
                    <Grid item xs={12} sm={6} key={fKey}>
                      <TextField
                        ...
                        variant="outlined"
                        size="small"
                        helperText={`Key: ${fKey} | Type: ${schemaField.type}`}
                      />
                    </Grid>
                  );
                })}
